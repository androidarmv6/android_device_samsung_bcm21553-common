...
static void select_devices(struct audio_device *adev, bool check)
{
    ...
    if (call_on) {
        if (speaker_on)
            audio_route_apply_and_update_path(adev->ar, "speaker-telephony");
        if (headphone_on)
            audio_route_apply_and_update_path(adev->ar, "headset-telephony");
        if (earpiece_on)
            audio_route_apply_and_update_path(adev->ar, "earpiece-telephony");
    } else {
        if (speaker_on)
    ...
    if (check)
        ALOGD("call=%c, earpiece=%c hp=%c speaker=%c dock=%c main-mic=%c", call_on ? 'y' : 'n', earpiece_on ? 'y' : 'n', headphone_on ? 'y' : 'n',
