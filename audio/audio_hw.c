...
#define IN_PERIOD_SIZE_LOW_LATENCY 160
...
int select_modeapp(struct audio_device *adev)
{
   int ret = 0;
   int fd;
   int call_on = adev->mode & AUDIO_MODE_IN_CALL;
   int buf[2] = {adev->audiomode, call_on ? 0 : 2}; // audioapp profile: 0 = voice call, 1 = voice call (WB), 2 = playback
...
int select_volume(int volume, int voltype)
{
   ...
   if (ret == -1)
      ALOGE("%s: ioctl %d failed", PCG_IOCTL_DEVICE, type);
...
static void select_devices(struct audio_device *adev)
{
    ...
    a2dp_on = adev->out_device & AUDIO_DEVICE_OUT_ALL_A2DP;

    audio_route_reset(adev->ar);
    audio_route_apply_and_update_path(adev->ar, "defaults");

    /* default volume configuration */
    adev->extamp_volume = false;
    adev->volume_scale = 4.16; // max: 24
    ...
    if (earpiece_on) {
        audio_route_apply_path(adev->ar, "earpiece");
        adev->audiomode = 0;
        adev->subdevice = 2;
    }
    if (headphone_on) {
    ...
    if (sco_on || a2dp_on) {
        audio_route_apply_path(adev->ar, "sco");
        adev->audiomode = 3;
        adev->subdevice = 6;
    }
    if (call_on)
    ...
    select_modeapp(adev);
    select_volume((100 / adev->volume_scale), adev->extamp_volume);

    ALOGD("call=%c, earpiece=%c hp=%c speaker=%c sco=%c a2dp=%c out_device=%d", call_on ? 'y' : 'n', earpiece_on ? 'y' : 'n', headphone_on ? 'y' : 'n',
          speaker_on ? 'y' : 'n', sco_on ? 'y' : 'n', a2dp_on ? 'y' : 'n', adev->out_device);
}
...
static int start_output_stream(struct stream_out *out)
{
    ...
    out->pcm = pcm_open(PCM_CARD, device, adev->subdevice, PCM_OUT | PCM_NORESTART | PCM_MONOTONIC, out->pcm_config);
...
static int adev_set_voice_volume(struct audio_hw_device *dev, float volume)
{
    ...
    ALOGV("voice volume: %.1f, extamp: %c, scale: %.1f, actual %d", volume, adev->extamp_volume ? 'y' : 'n', adev->volume_scale, scaledvol);
...
struct audio_module HAL_MODULE_INFO_SYM = {
    .common = {
        ...
        .name = "Samsung BCM21553 audio HW HAL",
