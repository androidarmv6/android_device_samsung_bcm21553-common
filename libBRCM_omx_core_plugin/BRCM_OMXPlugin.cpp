...
OMXPluginBase *createOMXPlugin() {
    return new BRCM_OMXPlugin;
}

BRCM_OMXPlugin::BRCM_OMXPlugin()
    : mLibHandle(dlopen("libBRCM_omx_core.so", RTLD_NOW)),
      mInit(NULL),
      mDeinit(NULL),
...
OMX_ERRORTYPE BRCM_OMXPlugin::makeComponentInstance(
        const char *name,
...
OMX_ERRORTYPE BRCM_OMXPlugin::destroyComponentInstance(
        OMX_COMPONENTTYPE *component) {
    if (mLibHandle == NULL) {
        return OMX_ErrorUndefined;
    }
...
OMX_ERRORTYPE BRCM_OMXPlugin::enumerateComponents(
        OMX_STRING name,
        size_t size,
        OMX_U32 index) {
    if (mLibHandle == NULL) {
...
OMX_ERRORTYPE BRCM_OMXPlugin::getRolesOfComponent(
        const char *name,
